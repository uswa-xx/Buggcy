...
window.addEventListener("DOMContentLoaded", () => {
    ...
    let colorPalete;
    let secretCode = [];
    colorPalete = document.querySelector("#color-palete");
    let gameSettings = {
        size: 4,
        max: 10,
        dim: 8
...
//Adds a specified number of circle elements to a container.
    const randomColor = () => {
    ...
    //Displays the secret pattern at the end of the game.
    ...
    //Colors the hint circles to indicate the accuracy of the player's guess.
    ...
    //Calculates the points where a new row of guesses should start.
    ...
    //Renders the game board based on the provided configuration.
    ...
    //Displays the game result (win/lose) and updates the background color accordingly.
...
//Generates a random pattern of colors.
    ...
    //Checks the player's guess against the secret pattern and updates the game state.
    const checkMyPatter = () => {
    ...
    //Removes the last selected color from the current guess.
    const removeColor = () => {
    ...
    const createNewGame = () => {
        ...
        console.log("Secret Code:", secretCode.map(colorIndex => colorPalete.childNodes[colorIndex].style.backgroundColor));
